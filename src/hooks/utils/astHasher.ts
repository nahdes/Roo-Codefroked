...
export async function hashCodeBlock(
  ...
  startLine?: number,
  endLine?: number
): Promise<ASTHashResult> { // ✅ FIXED: Added return type
  ...
  if (!parse) {
    console.warn(
      "[astHasher] @typescript-eslint/typescript-estree not found, falling back to raw hash"
    );
    return {
      hash:
        "raw-sha256:" +
...
type ASTNode = any; // ✅ FIXED: Use any for flexibility
...
function collectNodesInRange(
  ...
  function walk(node: ASTNode) {
    ...
    const inRange =
      startLine === undefined ||
      endLine === undefined ||
      (nodeLine !== undefined && nodeLine >= startLine && nodeLine <= endLine); // ✅ FIXED

    if (inRange && node.type) { // ✅ FIXED
      results.push(node);
    }
    ...
    for (const key of Object.keys(node)) {
      ...
      if (Array.isArray(child)) { // ✅ FIXED
        child.forEach((c) => c && typeof c === "object" && walk(c)); // ✅ FIXED
      } else if (child && typeof child === "object" && child.type) { // ✅ FIXED
        walk(child);
      }
    }
  }
...
function extractChildTypes(node: ASTNode): string[] {
  const types: string[] = [];
  for (const key of Object.keys(node)) {
    if (["type", "loc", "range", "parent", "start", "end"].includes(key))
      continue;
